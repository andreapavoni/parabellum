...
pub fn validate_marketplace_exchange_rules(
    ...
    if offer_resources.quantity == 0 || seek_resources.quantity == 0 {
        return Err(GameError::InvalidMarketplaceOffer);
    }

    if offer_resources.resource == seek_resources.resource {
        return Err(GameError::InvalidMarketplaceOffer);
    }

    let offer_total = offer_resources.quantity;
    let seek_total = seek_resources.quantity;
