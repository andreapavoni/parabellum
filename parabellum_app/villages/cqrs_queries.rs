//! CQRS query handlers for village read-model support.
//!
//! These are mini-CQRS query objects used by infrastructure when projecting or
//! reading ES-backed view state. They are not HTTP/API query ports; app-facing
//! query ports live with their owning app concerns.
...
mod tests {
    ...
    impl ReportRepository for MockReportRepo {
        async fn add_projected(
            &self,
            _report: &crate::villages::projection_repositories::ProjectedReport,
