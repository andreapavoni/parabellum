...
use parabellum_app::ports::queries::{
    MarketplaceData, TroopMovement, TroopMovementDirection, TroopMovementType,
...
use parabellum_app::villages::repositories::{ArmyRepository, MarketplaceRepository};
...
use crate::es::{
    PostgresArmyModelRepository, PostgresMarketplaceOfferRepository, VillageEsService,
};
...
impl VillageQueryPort for VillageEsAdapter {
    ...
    async fn get_marketplace_data(
        ...
        let marketplace_repo = PostgresMarketplaceOfferRepository::new(self.service.pool().clone());
    ...
    async fn get_village_army_state_view(
        ...
        let repo = PostgresArmyModelRepository::new(self.service.pool().clone());
        let home_army = repo
    ...
    async fn get_village_info_by_ids(
        ...
        for row in rows {
            ...
            out.insert(
                village_id,
                parabellum_app::query_models::VillageInfo {
                    id: village_id,
                    name: village_name,
                    position,
                },
    ...
    async fn get_leaderboard_page(
        ...
        for row in rows {
            ...
            entries.push(parabellum_app::query_models::PlayerLeaderboardEntry {
    ...
    async fn get_map_region(
        ...
        let fields = records
            ...
            .map(|record| {
                ...
                parabellum_app::query_models::MapRegionTile {
                    field: db_field.into(),
