...
pub mod tests {
    ...
    use parabellum_types::{Result, army::TroopSet, tribe::Tribe};
    ...
    async fn test_recall_deployed_troops() -> Result<()> {
        ...
        let units_to_deploy = TroopSet::new([100, 0, 0, 0, 0, 0, 0, 0, 0, 0]);
        ...
        let recall_command = RecallTroops {
            ...
            units: units_to_deploy.clone(), // Recall all units
        };
        ...
        // Verify troops returned home
        {
            ...
            assert_eq!(
                ...
                100,
                "Should have all 100 troops back"
            );
    ...
    async fn test_release_reinforcements() -> Result<()> {
        let (app, worker, uow_provider, _) = setup_app(false).await?;
        let units_to_send = TroopSet::new([50, 0, 0, 0, 0, 0, 0, 0, 0, 0]);
        ...
        let release_command = ReleaseReinforcements {
            ...
            units: units_to_send.clone(), // Release all units
        };
        ...
        // Verify reinforcements returned to source village
        {
            ...
            assert_eq!(
                home_army.units().get(0),
                50,
                "Should have all 50 troops back"
            );
    ...
    async fn test_recall_unauthorized_player() -> Result<()> {
        ...
        let reinforce_command = ReinforceVillage {
            ...
            units: TroopSet::new([100, 0, 0, 0, 0, 0, 0, 0, 0, 0]),
            target_village_id: target_village.id,
            hero_id: None,
        };
    ...
    async fn test_partial_recall_deployed_troops() -> Result<()> {
        let (app, worker, uow_provider, _) = setup_app(false).await?;
        let units_to_deploy = TroopSet::new([100, 50, 0, 0, 0, 0, 0, 0, 0, 0]);
        ...
        // Verify troops are deployed
        {
            ...
            assert_eq!(deployed_army.units().get(0), 100);
            assert_eq!(deployed_army.units().get(1), 50);
            uow.rollback().await?;
        }

        // Partial recall: recall only 60 legionnaires and 20 praetorians
        let units_to_recall = TroopSet::new([60, 20, 0, 0, 0, 0, 0, 0, 0, 0]);
        let recall_command = RecallTroops {
            player_id: deployer_player.id,
            village_id: deployer_village.id,
            army_id: deployed_army_id,
            units: units_to_recall.clone(),
        };
        ...
        let (recall_job, returning_army_id) = {
            ...
            assert_eq!(
                remaining_army.units().get(0),
                40,
                "Should have 40 legionnaires remaining (100 - 60)"
            );
            assert_eq!(
                remaining_army.units().get(1),
                30,
            ...
            assert_eq!(
                returning_army.units().get(0),
                60,
                "Returning army should have 60 legionnaires"
            );
            assert_eq!(
                returning_army.units().get(1),
                20,
    ...
    async fn test_partial_release_reinforcements() -> Result<()> {
        let (app, worker, uow_provider, _) = setup_app(false).await?;
        let units_to_send = TroopSet::new([80, 40, 0, 0, 0, 0, 0, 0, 0, 0]);
        ...
        // Verify reinforcements arrived
        {
            ...
            assert_eq!(reinforcement.units().get(0), 80);
            assert_eq!(reinforcement.units().get(1), 40);
            uow.rollback().await?;
        }

        // Partial release: release only 30 legionnaires and 15 praetorians
        let units_to_release = TroopSet::new([30, 15, 0, 0, 0, 0, 0, 0, 0, 0]);
        let release_command = ReleaseReinforcements {
            player_id: host_player.id,
            village_id: host_village.id,
            source_village_id: reinforcer_village.id,
            units: units_to_release.clone(),
        };
    ...
    async fn test_recall_with_invalid_quantities() -> Result<()> {
        let (app, worker, uow_provider, _) = setup_app(false).await?;
        let units_to_deploy = TroopSet::new([50, 0, 0, 0, 0, 0, 0, 0, 0, 0]);
